...
introPage <- tagList(
  ...
  h3("Background"),
  p("Wearable light loggers measure personal light exposure and are increasingly used to study how light exposure affects human physiology and behaviour. For example, researchers examine how light influences circadian rhythm entrainment (synchronisation of the body’s internal clock with the external light-dark cycle) and contributes to eye growth issues like myopia (nearsightedness). This guide is designed to help researchers make informed decisions when choosing a wearable light logger by outlining key device features and typical use cases in an accessible way."),
  ...
  p("Fidelity and accuracy describe the quality of the light measurements a logger provides. Important contributors to fidelity include the device’s spectral sensitivity (how well it matches standard α‑opic quantities – light metrics tied to human photoreceptor responses) and the range of light intensities it can record (capturing environments with very dim to very bright light conditions). The sensor’s directional response (how its sensitivity changes with the angle of incoming light) is another key factor. Additionally, the proximity of the sensor to the eye (the corneal plane) greatly affects fidelity – a sensor positioned at eye level will measure light similarly to what enters the eye, closely approximating the retinal irradiance that drives non‑visual biological responses to light."),
  ...
  p("For long‑term studies (many weeks to months or longer), researchers are for example interested in linking light exposure patterns with long‑term health outcomes or sleep patterns. In these cases, a measurement period of just a few days or weeks may not adequately represent an individual’s true long‑term light exposure – for example, a two‑week recording in winter could be vastly different from one in summer. Truly long‑term monitoring is often needed to capture seasonal and other gradual variations in light exposure. To date, very few studies have successfully tracked personal light exposure continuously over months or years alongside health outcomes, making this an emerging area of research."),
